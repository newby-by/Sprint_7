...
class TestLoginCourier:
    ...
    def test_login_courier_with_expected_data(self, existed_courier):
        ...
        assert (
            ...
            f"Data {existed_courier.generated_payload_for_login}"
            f" {response.text}"
        )
    ...
    def test_login_courier_with_wrong_data(self, existed_courier, method):
        ...
        assert (
            CourierMethods.get_status_code(response) == HTTPStatus.NOT_FOUND
            and
            CourierMethods.deserialize(response) ==
            CourierMethods.RESPONSE_LOGIN_COURIER_WITH_WRONG_DATA
        ), (
            f"Data {existed_courier.generated_payload_for_login}"
            f" {response.text}"
        )
        ...
        [data.Courier.generated_payload_without_login,
         pytest.param(data.Courier.generated_payload_without_password,
                      marks=pytest.mark.xfail(
                          reason='Without a pssword is FAILED'
                        ))]
    )
    def test_login_courier_without_required_data(self,
        ...
        assert (
            ...
            and
            CourierMethods.deserialize(response) ==
            CourierMethods.RESPONSE_LOGIN_COURIER_WITHOUT_REQUIRED_DATA
        ), (
